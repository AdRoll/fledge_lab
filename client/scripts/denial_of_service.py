...
parser.add_argument('--n-total', dest='n_total', type=int, default=100,
                    help='How many bidders to have in total')
...
# create list to determine how many malicious bidders we will be using and initialize a
# hashmap to record their respective times
...
for n_malicious in MALICIOUS_COUNTS:
    ...
    for i in reversed(range(normal_offset, args.n_total + 1)):
        if args.n_dsp > 1:
            dsp = get_dsp_to_use(i, args.n_dsp)
            browser.get(f"https://advertiser/dynamic?name=success-{i}&bid={i}&dsp={dsp}")
        else:
    ...
    for i in range(n_malicious):
        if args.n_dsp > 1:
            dsp = get_dsp_to_use(i, args.n_dsp)
            browser.get(f"https://advertiser/dynamic?name=ig-{i}-malicious&biddingLogicName=infinite_loop&dsp={dsp}")
        else:
...
g.set_title(
    f"Time taken to complete an auction and render the ad given n malicious bidders (from 0 to {args.n_total - 1})"
)
